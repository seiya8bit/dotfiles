...
2. Always run `git diff --check`, then pick from [README Verification](../../../README.md#verification):
   ...
   - Anything else under `home/` or `.github/`, `.chezmoiroot`, `.chezmoiversion` → both.
   - Docs and agent files only → every link, anchor, path and command they mention exists.
3. A check this machine cannot run (wrong OS, no Docker or chezmoi) is **not run**, never passed; say CI covers it.
